...
const string ReadInput[4] = {"name", "openingTime", "closingTime", "rank"};
enum hours
{
	QUARTER = 15,
	HALF = 30,
...
struct locations
{
>>>>>>> Change all functions and all constants
	int number;
	string name;
	int opening_time;
	int closing_time;
...
=======
vector<string> splitFirstLine(string firstline)
{
	vector<string> titles;
	string token;
	stringstream S(firstline);
...
int searchInVector(vector<string> vec, string element)
{
	for (int i = 0; i < vec.size(); i++)
		if (vec[i] == element)
			return i;
...
vector<int> arrangmentFunction(vector<string> titles)
{
	int name_index = searchInVector(titles, ReadInput[NAME_INDEX]);
	int opentime_index = searchInVector(titles, ReadInput[OPENINGTIME_INDEX]);
	int closetime_index = searchInVector(titles, ReadInput[CLOSINGTIME_INDEX]);
	int rank_index = searchInVector(titles, ReadInput[RANK_INDEX]);
	vector<int> title_arrangment = {name_index, opentime_index,
									closetime_index, rank_index};
	return title_arrangment;
}

vector<string> readFiles(string file_name)
{
	vector<string> input_strings;
	ifstream file(file_name);
	string temp_str;
...
vector<vector<string>> splitInput(vector<string> input_strings)
{
	vector<vector<string>> input_table;
...
vector<int> create_opentime_vector(vector<vector<string>>input_table, int opentime_index) {
	...
	for (int i = 0; i < location_number; i++) {
		...
		while (getline(S, token, ':')) {
=======
vector<int> createOpentimeVector(vector<vector<string>> input_table, int opentime_index)
{
	vector<int> open_times;
	string token;
	vector<int> temp;
...
vector<int> create_closetime_vector(vector<vector<string>>input_table, int closetime_index) {
	...
	for (int i = 0; i < location_number; i++) {
		...
		while (getline(S, token, ':')) {
=======
vector<int> createClosetimeVector(vector<vector<string>> input_table, int closetime_index)
{
	vector<int> close_times;
	string token;
	vector<int> temp;
...
=======
vector<locations> putInputToStruct(vector<vector<string>> input_table, vector<int> open_times, vector<int> close_times, vector<int> title_arrangment)
{
	vector<locations> input_structs;
	int string_num = input_table.size();
	for (int i = 0; i < string_num; i++)
	{
		vector<locations> input_vector;
		input_structs.push_back({i + 1, input_table[i][title_arrangment[0]], open_times[i], close_times[i], stoi(input_table[i][title_arrangment[RANK_INDEX]])});
>>>>>>> Change all functions and all constants
	}
...
=======
int existsInVector(vector<int> vec, int element)
{
	for (int i = 0; i < vec.size(); i++)
		if (vec[i] == element)
			return 1;
...
void findSuitableIndexs(vector<locations> input, int nearest_time, vector<int> location_check, vector<int> &suitable_indexs, vector<int> unsuitable_indexs)
{
	...
	for (int i = 0; i < size; i++)
	{
		if (input[i].opening_time == nearest_time)
		{
			int existence_checker = existsInVector(location_check, (input[i].number - 1));
			int unsuitable_checker = existsInVector(unsuitable_indexs, i);
			if (existence_checker == 0 && unsuitable_checker == 0)
				suitable_indexs.push_back(i);
		}
	}
...
int matchNumRank(vector<locations> input, int rank)
{
	for (int i = 0; i < input.size(); i++)
		if (input[i].rank == rank)
			return i;
...
int findBest(vector<int> suitable_indexs, vector<locations> input)
{
	vector<int> ranks;
	int index;
...
vector<int> findOpenLocs(vector<int> open_times, int current_time, vector<int> location_check, vector<int> unsuitable_indexs)
{
	vector<int> suitable_indexs;
	for (int i = 0; i < open_times.size(); i++)
	{
		int existence_checker = existsInVector(location_check, i);
		if (open_times[i] <= current_time && existence_checker == 0)
		{
			int unsuitable_check = existsInVector(unsuitable_indexs, i);
			if (unsuitable_check == 0)
				suitable_indexs.push_back(i);
		}
	}
...
int findNextDestinationIndex(int &current_time, vector<int> open_times, vector<locations> input, vector<int> location_check, vector<int> unsuitable_indexs)
{
	...
	while (true)
	{
		// we stay in this while till suitable indexes is empty
		suitable_indexs.clear();
		if (current_time >= findMin(open_times) && counter < len)
		{
			suitable_indexs = findOpenLocs(open_times, current_time, location_check, unsuitable_indexs);
			counter += 1;
		}
		else
		{
			...
			int nearest_time = findMin(late_opentimes);
			findSuitableIndexs(input, nearest_time, location_check, suitable_indexs, unsuitable_indexs);
			current_time = nearest_time;
		}
		if (suitable_indexs.size() > 0)
		{
			rank = findBest(suitable_indexs, input);
			index = matchNumRank(input, rank);
>>>>>>> Change all functions and all constants
			break;
		}
	}
...
int checkDestinationWellness(vector<locations> input, int current_time, int index)
{
	int duration = input[index].closing_time - current_time;
...
int findMax(vector<int> closing_time)
{
	int size = closing_time.size();
	if (size == 1)
		return closing_time[0];
	else
	{
		int tail = closing_time[size - 1];
		closing_time.pop_back();
		if (tail >= findMax(closing_time))
			return tail;
		else
			return findMax(closing_time);
	}
}
...
void findNextDestination(vector<locations> input, vector<int> &location_check, vector<int> &start, vector<int> &durations, int current_time = 480)
{
	// TODO: fix this function
	int size = input.size();
	vector<int> not_suitables;
	int counter = 0;
	while (current_time < findMax(close_times) && counter < size)
	{
		int index = findNextDestinationIndex(current_time, open_times, input, location_check, not_suitables);
		int existence_checker = existenceCheck(location_check, index);
		int duration_check = checkDestinationWellness(input, current_time, index);
		if (existence_checker == (-1) && duration_check != (-1))
		{
			location_check.push_back(index);
			start.push_back(current_time);
...
string convertIntToClockform(int time)
{
	int hour = time / HOUR;
...
=======
vector<vector<string>> makeVectorReadyForPrint(vector<locations> input, vector<int> close_times, vector<int> &location_check, vector<int> &start, vector<int> &durations)
{
	vector<vector<string>> temp_vector;
	for (int i = 0; i < location_check.size(); i++)
	{
		int end = calculate_endtime(start[i], durations[i]);
		string standard_start = convertIntToClockform(start[i]);
		string standard_end = convertIntToClockform(end);
		vector<string> temp = {input[location_check[i]].name, standard_start, standard_end};
		temp_vector.push_back(temp);
	}
	return temp_vector;
...
void printOutput(vector<vector<string>> temp_vector)
{
	for (int i = 0; i < temp_vector.size(); i++)
	{
...
vector<locations> readFromFile(string file_name)
{
	ifstream file(file_name);
	string temp_str;
	getline(file, temp_str);
	...
	while (getline(S, token, ','))
		splitted_firstline.push_back(token);
	vector<int> arrangment = arrangmentFunction(splitted_firstline);
	vector<string> primitive_get = readFiles(file_name);
	vector<vector<string>> splitted_input = splitInput(primitive_get);
	open_times = createOpentimeVector(splitted_input, arrangment[1]);
	close_times = createClosetimeVector(splitted_input, arrangment[2]);
	vector<locations> location_data = putInputToStruct(splitted_input, open_times, close_times, arrangment);
	return location_data;
}
...
int main(int argc, char *argv[])
{
	...
	vector<int> start_times;
	vector<int> durations;
	vector<locations> location_data = readFromFile(argv[1] + 2);
	findNextDestination(location_data, gone_location, start_times, durations);
	vector<vector<string>> ready_to_print = makeVectorReadyForPrint(location_data, close_times, gone_location, start_times, durations);
	printOutput(ready_to_print);
>>>>>>> Change all functions and all constants
}
